...
- **每日自动签到**：定时运行脚本完成每日签到，领取成长奖励。
  - **每日两次签到尝试**：分别在北京时间早上 9 点和下午 1 点左右尝试签到，增加成功率。
...
3. **Workflow 显示跳过（Skipped）**
   * 这是预期行为。如果当天的第一次签到尝试（例如早上 9 点的任务）已经成功，那么下午 1 点的任务在检查时会发现已签到，从而跳过实际的签到步骤。你可以在 `check-if-already-signed` job 的日志中看到类似“今天已成功签到，跳过执行”的信息。
